...
function LibraryPage({ highlightAlbumId, albumDetailId = null, onOpenAlbum }: LibraryPageProps) {
    const { t } = useI18n()
    ...
    const detailView = albumDetailId ? (
        <AlbumDetailPage
            ...
            isLoading={isLibraryLoading || isRotationLoading || isListenLoading}
            partialErrors={[libraryError, listenError, rotationError].filter((value): value is string => Boolean(value))}
            onBack={() => onOpenAlbum?.(albumDetailId)}
