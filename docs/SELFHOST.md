...
also rejects requests explicitly marked `Sec-Fetch-Site: cross-site` by the browser.
It deliberately does not compare `Origin` with the upstream Host or protocol because
NAS reverse proxies may legitimately rewrite all of them before forwarding the
request to Caddy.
