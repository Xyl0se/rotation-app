...
Every pull request and every push to `main` runs the reusable `Validate` workflow. It installs both npm packages with `npm ci`, lints and tests frontend and server in their own environments, builds both packages, validates development and production Compose files, and starts the development stack for HTTP health checks.
